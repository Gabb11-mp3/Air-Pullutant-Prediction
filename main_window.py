...
from file_upload import choose_csv_file, load_and_clean_data
...
class AirPollutionWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.dataset_path = Path(DEFAULT_CSV_FILE)
        ...
        # Loading after the event loop starts allows the window to appear
        # before a large dataset is processed.
    ...
    def build_interface(self) -> None:
        ...
        subtitle = QLabel(
            ...
            "and linear-regression forecasts"
        )
    ...
    def _build_controls(self) -> QFrame:
        """Build the dataset and prediction-year controls."""
        controls = QFrame()
        ...
        prediction_year_label = QLabel("Prediction year")
        prediction_year_label.setObjectName("FieldLabel")
    ...
    def _build_results_panel(self) -> QFrame:
        ...
        information_note = QLabel(
            "Forecasts use linear regression trained with gradient descent. "
            "The shaded 95% prediction range grows as uncertainty increases. "
            "Treat forecasts as estimates rather than measured values."
        )
        information_note.setObjectName("InfoNote")
        information_note.setWordWrap(True)

        layout.addWidget(results_title)
        layout.addWidget(self.summary_label)
        layout.addWidget(self.prediction_table, 1)
        layout.addWidget(information_note)
        return results_card

    def _build_graph_panel(self) -> QFrame:
        """Build the Matplotlib toolbar and scrollable graph panel."""
        graph_card = QFrame()
        graph_card.setObjectName("Card")
        layout = QVBoxLayout(graph_card)
        layout.setContentsMargins(10, 10, 10, 10)
        layout.setSpacing(6)

        self.canvas = PollutionChartCanvas(graph_card)
        self.toolbar = NavigationToolbar2QT(self.canvas, graph_card)
        self.toolbar.setObjectName("ChartToolbar")

        graph_scroll = QScrollArea()
        graph_scroll.setObjectName("ChartScroll")
    ...
    def choose_dataset(self) -> None:
        """Ask the user for a CSV and refresh the dashboard."""
        selected_path = choose_csv_file(self, self.dataset_path)
    ...
    def generate_predictions(self) -> None:
        """Load the active dataset, calculate results, and refresh the UI."""
        ...
        try:
            ...
            if skipped:
                summary += (
                    "\nSkipped because of insufficient data: "
                    + ", ".join(skipped)
                )

            self.summary_label.setText(summary)
            self.statusBar().showMessage(
                f"Dashboard updated for {target_year}",
                6000,
            )
    ...
    def update_prediction_table(
        ...
        """Replace the table contents with the latest predictions."""
        self.prediction_table.setRowCount(len(predictions))

        for row, (pollutant, concentration) in enumerate(
            predictions.items()
        ):
    ...
    def show_error(self, title: str, message: str) -> None:
        """Show an expected data or prediction error."""
